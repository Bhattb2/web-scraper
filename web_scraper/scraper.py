...
def _get_soup(URL):
    """
    Helper function to handle requests and parse them into BeautifulSoup objects
    """
...
def _general_citations_needed(URL):
    """
    Some wikipedia pages require citations but do not state precisely where they are needed. This is the helper function to handle those cases
    """
    soup = _get_soup(URL)
...
def get_citations_needed_count(URL):
    """
    Return a count of how many "citations needed" flags there are on ant given wikipedia page
    """
    soup = _get_soup(URL)
...
def get_citations_needed_report(URL):
    ...
    articles = "---- Citations need report ----\n"
    count = 0
    for result in results:
        if result.text == 'cite':
            count += 1
